...
# ── Agent preset (presets/chat.md) ──────────────────────────────────


class AgentRulesResponse(BaseModel):
    """System prompt preset content."""

    model_config = ConfigDict(extra="forbid")

    content: str = Field("", description="Raw markdown content of the preset.")
    path: str = Field("", description="Absolute path to the preset file.")


class AgentRulesInput(BaseModel):
    """PUT body for preset update."""

    model_config = ConfigDict(extra="forbid")

    content: str = Field(..., description="New markdown content for the preset.")


def _preset_path(state: AppState, name: str = "chat") -> str | None:
    """Resolve a preset path from the skills root."""
    import os

    if state.skill_host is not None:
        repo_root = str(state.skill_host.skills_root.parent)
        return os.path.join(repo_root, "presets", f"{name}.md")
    root = os.environ.get("AAF_REPO_ROOT", "")
    if root:
        return os.path.join(root, "presets", f"{name}.md")
    return None


@router.get(
    "/agent",
    response_model=AgentRulesResponse,
    summary="Read the chat system prompt preset (presets/chat.md)",
)
async def get_agent_rules(
    state: Annotated[AppState, Depends(get_app_state)],
) -> AgentRulesResponse:
    path = _preset_path(state) or ""
    content = ""
    if path and __import__("os").path.isfile(path):
        try:
            ...
            pass
    return AgentRulesResponse(content=content, path=path)


@router.put(
    "/agent",
    response_model=AgentRulesResponse,
    summary="Update the chat system prompt preset (persists to disk)",
)
async def put_agent_rules(
    payload: AgentRulesInput,
    state: Annotated[AppState, Depends(get_app_state)],
    user: Annotated[User, Depends(require_admin_or_open_mode)],
) -> AgentRulesResponse:
    path = _preset_path(state)
    if not path:
        raise HTTPException(status_code=503, detail="Cannot resolve preset path")
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to write preset: {exc}") from exc
    log.info("settings.preset.updated", path=path)
    return AgentRulesResponse(content=content, path=path)
