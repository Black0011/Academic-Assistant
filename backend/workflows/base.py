...
class WorkflowContext:
    ...
    manuscripts: Any | None = None
    bundle_storage: Any | None = None
    store: Any | None = None
    ...
    async def ask_user(
        ...
        *,
        ...
        ``TASK_AWAITING_INPUT`` event and sets a pause marker on state.
        The runner detects the marker, stores the snapshot, and marks
        the task as ``"waiting"``.

        On the resume call (``_resume_checkpoint`` in ``input`` matches
        ``checkpoint``): restores ``self.state`` from the saved snapshot
        and returns the user's response dict.
