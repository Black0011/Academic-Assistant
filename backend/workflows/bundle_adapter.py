...
class BundleAdapter:
    ...
    # P18: LRU file cache — avoids repeated disk I/O for hot files.
    # Key = (manuscript_id, path), stores (mtime, content).
    _read_cache: dict = field(default_factory=dict)
    _read_cache_max: int = 64  # ~5 MB at typical paper file sizes
    ...
    async def read_text(self, rel_path: str, *, encoding: str = "utf-8") -> str:
        ...
        """
        cache_key = (self.manuscript.id, rel_path)
        cached = self._read_cache.get(cache_key)
        if cached is not None:
            cached_mtime, cached_content = cached
            try:
                stat = await self.storage.stat(self.manuscript, rel_path)
                if stat.modified_at.timestamp() <= cached_mtime:
                    return cached_content
            except Exception:
                pass  # stat failed → bypass cache, re-read from disk
            del self._read_cache[cache_key]
        content = await self.storage.read_text(self.manuscript, rel_path, encoding=encoding)
        ...
        try:
            stat = await self.storage.stat(self.manuscript, rel_path)
            self._read_cache[cache_key] = (stat.modified_at.timestamp(), content)
        except Exception:
            pass  # can't cache without mtime
        return content
    ...
    async def write_text(
        ...
        *,
        ...
        self._read_cache.pop((self.manuscript.id, rel_path), None)
