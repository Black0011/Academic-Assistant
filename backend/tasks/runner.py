...
async def execute_task(task_id: str, deps: RunnerDeps) -> None:
    ...
    if (
        ...
        and workflow_name in {"project-consult", "project-revision", "auto"}
    ...
    if (
        bundle_adapter is not None
        and workflow_name in {"revision", "consult", "peer-review", "project-revision", "auto"}
        and not workflow_input.get("text")
    ):
        targets: list[str] = []
        ...
        if not targets and workflow_name in {"peer-review", "consult", "auto"}:
            try:
                manifest = await bundle_adapter.list_tree()
    ...
    ctx = WorkflowContext(
        ...
        manuscripts=deps.manuscripts,
        bundle_storage=deps.bundle_storage,
        store=store,
        budget=budget,
    )
    ...
    if out.verdict == "waiting":
        ...
        await store.mark_completed(task_id, status="waiting", result=results, budget=budget.snapshot())
        await sink(Event(
