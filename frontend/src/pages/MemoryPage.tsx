...
function RollbackTab() {
  ...
  const rollback = useMutation({
    mutationFn: () => memoryApi.rollbackRun(runId.trim()),
    onSuccess: (res) => {
      toast.success(
        `Rolled back run ${res.run_id}: ` +
          `−${res.knowledge_removed} knowledge, ` +
          `−${res.heuristics_removed} heuristics, ` +
          `−${res.reflections_removed} reflections`,
      );
      void qc.invalidateQueries({ queryKey: ["memory"] });
    },
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-sm">
          <RotateCcw className="h-4 w-4" /> Rollback a run
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        <p className="text-sm text-[var(--color-muted-foreground)]">
          Removes every memory write tagged with the given <code>run_id</code> across knowledge,
          heuristics, and reflections. This cannot be undone — keep a copy of the run id you want
          to nuke.
        </p>
        <div className="grid gap-3 sm:grid-cols-[1fr_auto]">
          <div className="flex flex-col gap-1">
            <Label htmlFor="rb-run">Run ID</Label>
            <Input
              id="rb-run"
              value={runId}
              onChange={(e) => setRunId(e.target.value)}
              placeholder="e.g. task-abc123 or workflow run id"
            />
          </div>
          <Button
            variant="destructive"
            onClick={() => {
              ...
              if (
                confirm(
                  `Rollback all memory writes from run "${runId.trim()}"? This is irreversible.`,
                )
              ) {
                rollback.mutate();
              }
            }}
            disabled={!runId.trim() || rollback.isPending}
            className="self-end"
          >
        ...
        {rollback.data && (
          <div className="rounded-md border bg-[var(--color-background)] p-4 text-sm">
            <p className="font-medium">Last rollback</p>
            <dl className="mt-2 grid gap-1 text-xs sm:grid-cols-4">
              <DefRow label="Run">{rollback.data.run_id}</DefRow>
              <DefRow label="Knowledge">−{rollback.data.knowledge_removed}</DefRow>
              <DefRow label="Heuristics">−{rollback.data.heuristics_removed}</DefRow>
              <DefRow label="Reflections">−{rollback.data.reflections_removed}</DefRow>
            </dl>
          </div>
        )}
      </CardContent>
