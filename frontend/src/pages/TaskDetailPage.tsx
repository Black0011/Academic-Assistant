...
import { ArrowLeft, Code, ExternalLink, Loader2, MessageCircleMore, Send, Sparkles, Wrench } from "lucide-react";
...
import { ToolCallCard, type ToolCallStep } from "@/components/chat/ToolCallCard";
...
export function TaskDetailPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Process — execution steps, tool/skill calls, thinking ------------ */}
      <Card>
        <CardHeader>
        ...
        <CardContent>
          ...
          {!isTerminal && events.length === 0 && (
            <div className="flex items-center gap-2 py-4 text-xs text-[var(--color-muted-foreground)]">
              <Loader2 className="h-3.5 w-3.5 animate-spin" />
              {taskQ.data?.status === "queued"
                ? "Task queued — waiting to start…"
                : "Agent is working…"}
            </div>
          )}
          {isTerminal && <DoneProcess task={task} events={events} />}
        </CardContent>
      </Card>

      {/* Result — final answer (auto workflow) or workflow-specific panel --- */}
      {task.workflow === "auto" ? (
        <AutoAnswerCard task={task} />
      ) : (
...
// ── Auto workflow result panel — clean answer + tool calls + reasoning ──

// ── Unified process view — works with live events or persisted data ──

// ── Done: execution steps + reasoning (Process card) ──────────────

function DoneProcess({ task, events }: { task: TaskRecord; events: StreamEvent[] }) {
  const r = (task.result ?? {}) as Record<string, unknown>;
  const reasoning = (r.reasoning as string | undefined) ?? "";
  const toolCalls = (r.tool_calls as Array<{name: string; kind: string; args: Record<string,unknown>; result_summary: string}> | undefined) ?? [];

  const steps: ToolCallStep[] = toolCalls.map((tc, i) => ({
    id: `${tc.name}-${i}`,
    kind: (tc.kind === "skill" ? "skill" : tc.kind === "mcp" ? "mcp" : tc.kind === "file_read" ? "file_read" : tc.kind === "file_write" ? "file_write" : tc.kind === "script" ? "script" : "thinking") as ToolCallStep["kind"],
    name: tc.name,
    status: "done" as const,
    summary: tc.result_summary?.slice(0, 120),
    args: tc.args,
  }));

  if (events.length === 0 && steps.length === 0 && !reasoning) {
    return <p className="text-xs italic text-[var(--color-muted-foreground)]">No execution records.</p>;
  }
    ...
    <div className="space-y-3">
      ...
      {steps.length > 0 && (
        <div className="rounded border border-[var(--color-border)]/50 p-2">
          ...
          <ToolCallCard steps={steps} />
        </div>
      )}
      {reasoning && (
        <details className="rounded border border-[var(--color-border)]/50 bg-[var(--color-muted)]/10 p-2">
          ...
          <pre className="mt-1.5 max-h-48 overflow-y-auto whitespace-pre-wrap rounded bg-[var(--color-muted)]/20 p-2 font-mono text-[10px] leading-relaxed text-[var(--color-muted-foreground)]">
            {reasoning}
          </pre>
        </details>
      )}
...
// ── Auto workflow: final answer only (separate from Process) ──────

function AutoAnswerCard({ task }: { task: TaskRecord }) {
  const r = (task.result ?? {}) as Record<string, unknown>;
  const answer = (r.answer as string) ?? "";
  const toolRounds = (r.tool_rounds as number | undefined) ?? 0;
  ...
  if (!answer) {
    return (
      <Card>
        ...
        <CardContent>
          <p className="text-xs italic text-[var(--color-muted-foreground)]">No answer produced.</p>
        </CardContent>
      </Card>
    );
  }
        ...
        <div className="flex items-center justify-between">
          ...
          {toolRounds > 0 && (
            <span className="text-[10px] text-[var(--color-muted-foreground)]">
              {toolRounds} LLM round{toolRounds > 1 ? "s" : ""}
            </span>
          )}
        </div>
      </CardHeader>
      <CardContent>
        <pre className="whitespace-pre-wrap rounded bg-[var(--color-background)] p-3 font-mono text-[12px] leading-relaxed text-[var(--color-foreground)]">
          {answer}
        </pre>
      </CardContent>
    </Card>
